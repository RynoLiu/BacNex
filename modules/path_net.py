...
# saved in tmp_files
path_subnet_edge.to_csv(os.path.join(tmp_dir, f"{target_label}_{target_pathway}_edge.csv"), index=False)
path_subnet_node.to_csv(os.path.join(tmp_dir, f"{target_label}_{target_pathway}_node.csv"), index=False)

# the KO set exist in taxa features and target pathway, used for KEGG pathway visualization
with open(os.path.join(tmp_dir, f"{target_label}_{target_pathway}_koset.txt"), "w") as fres:
    for ko in subnet_ko:
        fres.write(f"{ko}\n")